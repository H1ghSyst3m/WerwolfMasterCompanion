...
Online Mode
    ...
    │       └── Players can reopen their private role card, including role info
    │
    └── Game Over
...
- The current dawn report does not list Harter Bursche as dead unless another non-wolf effect kills him.
...
- During the next `resolveNight()`, if he is still alive, he dies from the pending wound at the following dawn.
...
**Online behavior:** Online Mode always uses player-owned role reveal. Each player reveals their own role on their own phone, can view a less visually revealing role description from the footer after the private card has been revealed once, and the GM sees reveal completion status.
